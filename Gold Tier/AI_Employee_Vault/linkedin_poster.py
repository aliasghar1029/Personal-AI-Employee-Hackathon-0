...
class LinkedInPoster:
    ...
    def _initialize(self):
        ...
        # Check if we need to authenticate (no token or invalid token)
        if not self.access_token or self.access_token.strip() == '':
            logger.info("No valid LinkedIn access token found")
            self._delete_access_token()
            self._authenticate()
        else:
            # Validate existing token
            if not self._validate_access_token():
                logger.info("LinkedIn access token expired, refreshing...")
                self._delete_access_token()
                self._authenticate()
    ...
    def _validate_access_token(self) -> bool:
        ...
        try:
            ...
            # Try to get user info - this will fail if token is expired
            ...
            if response.status_code == 200:
                ...
                logger.info(f"LinkedIn token validated successfully (User ID: {self.user_id})")
                return True
            elif response.status_code == 401:
                logger.info("LinkedIn token expired (401 Unauthorized)")
                return False
            else:
                logger.warning(f"LinkedIn token validation returned: {response.status_code}")
    ...
    def _delete_access_token(self):
        ...
        if ENV_PATH.exists():
            ...
            logger.info("Old access token removed from .env file")
